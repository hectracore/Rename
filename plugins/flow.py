...
@Client.on_callback_query(filters.regex(r"^convert_to_(mp3|gif|mkv|mp4|png|jpg|webp)$"))
async def handle_convert_to(client, callback_query):
    user_id = callback_query.from_user.id
    target_format = callback_query.data.split("_")[2]

    await callback_query.message.edit_text(f"Processing conversion to {target_format.upper()}...", quote=True)
...
async def handle_file_upload(client, message):
    ...
    if state == "awaiting_convert_file":
        ...
        buttons = []
        if is_video:
            if ext != "mp3": buttons.append([InlineKeyboardButton("Extract Audio (MP3)", callback_data="convert_to_mp3")])
            if ext != "gif": buttons.append([InlineKeyboardButton("Convert to GIF", callback_data="convert_to_gif")])
            if ext != "mkv": buttons.append([InlineKeyboardButton("Convert to MKV", callback_data="convert_to_mkv")])
            if ext != "mp4": buttons.append([InlineKeyboardButton("Convert to MP4", callback_data="convert_to_mp4")])
        elif is_image:
            row1 = []
            if ext != "png": row1.append(InlineKeyboardButton("Convert to PNG", callback_data="convert_to_png"))
            if ext not in ["jpg", "jpeg"]: row1.append(InlineKeyboardButton("Convert to JPG", callback_data="convert_to_jpg"))
            if row1: buttons.append(row1)

            row2 = []
            if ext != "webp": row2.append(InlineKeyboardButton("Convert to WEBP", callback_data="convert_to_webp"))
            # User decided against many formats, keeping it to PNG, JPG, WEBP. Removed PDF.
    ...
    if state == "awaiting_audio_file":
        ...
        from plugins.flow import render_audio_menu
    ...
    if state == "awaiting_general_file":
        ...
        elif getattr(message, "audio", None): file_name = message.audio.file_name
    ...
    media = getattr(message, "document", None) or getattr(message, "video", None) or getattr(message, "audio", None) or getattr(message, "voice", None)
    if media:
